...
long timeSegmentMs = timeSegmentMin * 60000; // 900000 is 15 min
...
void checkTouches() {
  ...
  for (int i = 1; i < subjectsUsed; i++) {
    if ((currentTouch != previousTouch) && (previousTouch == subject[i].beginLed) && (abs(previousTouch - currentTouch) < 2)) {
  ...
  for (int i = 0; i < subjectsUsed - 1; i++) {
    if ((currentTouch != previousTouch) && (previousTouch == subject[i].endLed) && (abs(previousTouch - currentTouch) < 2)) {
      directionValue = currentTouch - previousTouch;
...
void sendToDailyMeet() {
    String nextmeeting = "";
    nextmeeting += timeSegmentsOn;
    nextmeeting += ".";
    nextmeeting += plusminus;

    Serial1.write(nextmeeting.c_str());
    Serial1.write('\n');
    plusminus = 0;
}
